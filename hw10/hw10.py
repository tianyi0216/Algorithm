...
if __name__ == '__main__': 
    ...
    for i in range(num_instance):
        ...
        # modification to previous week's homework.
        # make all edge capacity 1, add extra edge from s to set a, from set b to t
        g = Graph()
        for edge in edges:
            edge[1] = f"{edge[1]}b"
            edge.append(1)
            g.add_edge(edge)
            if ("s" not in g.nodes) or ("s" in g.nodes and edge[0] not in g.nodes["s"].childs):
                g.add_edge(["s", edge[0], 1])
            if ("t" not in g.nodes) or ("t" not in g.nodes[edge[1]].childs):
                g.add_edge([edge[1], "t", 1])

        res = g.max_flow("t")
        result.append(f"{res} Y" if res == a_node and res == b_node else f"{res} N") 
    # print all results
    for res in result:
